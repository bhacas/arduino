...
static byte mac[] = { 0x74,0x69,0x69,0x2D,0x31,0x02 };
IPAddress server(192, 168, 1, 10);
EthernetClient ethClient;
PubSubClient client(ethClient);

#define SERIAL 0
#define STATIC 0 // set to 1 to disable DHCP (adjust myip/gwip values below)home/oliwia
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr == "home/oliwia/light1/set") {
    if ((char)payload[0] == '0') {
      switchOutput("light1", 0, true);
    } else if ((char)payload[0] == '1') {
      switchOutput("light1", 1, true);
    } 
  } else if (topicStr == "home/oliwia/light2/set") {
    if ((char)payload[0] == '0') {
      switchOutput("light2", 0, true);
    } else if ((char)payload[0] == '1') {
      switchOutput("light2", 1, true);
    }
  } else if (topicStr == "home/oliwia/blind/set") {
    if ((char)payload[0] == '0') {
  ...
  } else if (topicStr == "home/oliwia/blind/position") {
...
boolean reconnect() {
  Ethernet.begin(mac);
  delay(1500);
  if (client.connect("oliwia")) {
    publishFloat("home/oliwia/temp", (temp*10));
    publishFloat("home/oliwia/humidity", humidity);
    publishFloat("home/oliwia/light1", light1);
    publishFloat("home/oliwia/switch2", switch2);
    publishFloat("home/oliwia/blind", blindState);
    publishFloat("home/oliwia/contactron1", contactron1);
    publishFloat("home/oliwia/contactron2", contactron2);
    publishFloat("home/oliwia/contactron3", contactron3);
    client.subscribe("home/oliwia/light1/set");
    client.subscribe("home/oliwia/light2/set");
    client.subscribe("home/oliwia/blind/set");
    client.subscribe("home/oliwia/blind/position");
  }
...
void switchOutput(String type, float value, boolean doNotifyMaster) {
  ...
  if (type == "light2") {
    ...
    publishFloat("home/oliwia/light2", light2); 
  }
