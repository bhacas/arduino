...
static byte mac[] = { 0x74,0x69,0x69,0x2D,0x31,0x03 };
IPAddress server(192, 168, 1, 10);
EthernetClient ethClient;
PubSubClient client(ethClient);

#define SERIAL 0
#define STATIC 0 // set to 1 to disable DHCP (adjust myip/gwip values below)home/lazienka
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr == "home/lazienka/light1/set") {
    if ((char)payload[0] == '0') {
      switchOutput("light1", 0, true);
    } else if ((char)payload[0] == '1') {
      switchOutput("light1", 1, true);
    } 
  } else if (topicStr == "home/lazienka/light2/set") {
    if ((char)payload[0] == '0') {
      switchOutput("light2", 0, true);
    } else if ((char)payload[0] == '1') {
      switchOutput("light2", 1, true);
    }
  } else if (topicStr == "home/lazienka/blind/set") {
    if ((char)payload[0] == '0') {
  ...
  } else if (topicStr == "home/lazienka/blind/position") {
...
boolean reconnect() {
  if (client.connect("lazienka")) {
    publishFloat("home/lazienka/temp", (temp*10));
    publishFloat("home/lazienka/humidity", humidity);
    publishFloat("home/lazienka/light1", light1);
    publishFloat("home/lazienka/switch2", switch2);
    publishFloat("home/lazienka/blind", blindState);
    publishFloat("home/lazienka/contactron1", contactron1);
    publishFloat("home/lazienka/contactron2", contactron2);
    publishFloat("home/lazienka/contactron3", contactron3);
    client.subscribe("home/lazienka/light1/set");
    client.subscribe("home/lazienka/light2/set");
    client.subscribe("home/lazienka/blind/set");
    client.subscribe("home/lazienka/blind/position");
  }
...
void switchOutput(String type, float value, boolean doNotifyMaster) {
  ...
  if (type == "light2") {
    ...
    publishFloat("home/lazienka/light2", light2); 
  }
